...
\begin{abstract}
No single approach to sea level rise (SLR) projection is sufficient.  Process models lack systematic parameter exploration and diverge from observations; purely statistical models cannot resolve component-specific dynamics; expert assessments lack formal self-consistency.  We outline a hierarchical Bayesian framework that fuses observational constraints, statistical models, and process-model structural information into a single, self-consistent probabilistic forecast.  The framework operates across four levels: (0)~global temperature forcing, (1)~total GMSL constrained by the observational record, (2)~component-level decomposition with component-specific data and models, and (3)~deep-uncertainty modules for ice-sheet processes.  At each level, posteriors from one analysis become informed priors for the next, and a global budget constraint ensures that components sum to the observed total.  Forecast uncertainty is decomposed by horizon: at short lead times ($<$15 years), observational constraints dominate; at intermediate horizons (15--60 years), physics-informed component models under diverging forcing scenarios provide the primary structure; at long horizons ($>$60 years), deep-uncertainty modules for Antarctic ice-sheet processes and scenario divergence dominate the projection envelope.  The appropriate forecasting methodology and the dominant uncertainty source both change with horizon, and the framework makes this transition explicit and quantifiable.  The framework is designed to be built incrementally: it is self-consistent at every stage of completeness, with unresolved components absorbed into a residual term.
\end{abstract}
...
\subsection{Multi-timescale forecasting}

A second organising principle is that no single forecasting methodology is adequate across all time horizons.  At short lead times ($\tau < 15$~years), the system trajectory is tightly constrained by its current state and the rate of recent acceleration; observationally calibrated rate models outperform process models, which carry substantial initialisation errors.  At intermediate horizons ($15 < \tau < 60$~years), the forcing scenarios diverge, component models extrapolate beyond their calibration domain, and the structural uncertainty grows; the appropriate methodology is physics-informed projection with growing model-inadequacy terms.  At long horizons ($\tau > 60$~years), deep uncertainty in ice-sheet dynamics and scenario divergence dominate the projection envelope, and the credibility basis shifts from empirical validation to physical plausibility, conservation laws, and expert assessment.

The framework addresses this by integrating observation-constrained, physics-informed, and deep-uncertainty methods with horizon-appropriate weighting.  Rather than prescribing different models at different horizons, the framework uses a single joint model whose variance decomposition (\cref{sec:variance}) reveals which uncertainty source dominates at each horizon.  The transitions between regimes are not set by fiat --- they emerge from the posterior as the relative magnitudes of parameter uncertainty, scenario uncertainty, model inadequacy, and deep uncertainty shift with lead time.  This makes the framework's own credibility as a function of horizon transparent and quantifiable (\cref{sec:forecast-regimes}).
...
\end{equation}
incorporates both the measurement uncertainty in the total GMSL rate and the component-level model inadequacy terms $\sigma_{\text{extra},i}$ (see~\cref{sec:variance}).  In the limit $\sigma_{\text{budget}} \to 0$, this recovers the deterministic budget; allowing slight imbalance prevents numerical pathologies from degenerate posteriors while honestly reflecting that no finite set of component models perfectly accounts for every physical process contributing to the total.
...
\paragraph{Sequential update interface (Phase~1 design, Phase~2 implementation).}
The principle that forecasts should be updated as new observations arrive requires formal support.  New data arrive annually (GMSL from altimetry, GRACE mass-change estimates, Argo thermosteric profiles), and the framework must incorporate this information without full re-estimation at each update.  Three update algorithms are available in order of increasing computational cost: (A)~importance reweighting of the stored MCMC sample, (B)~sequential Monte Carlo (particle filter with rejuvenation), and (C)~full MCMC re-estimation.

Option~A is the Phase~1 target.  Given the stored posterior sample $\{\boldsymbol{\theta}^{(m)}\}_{m=1}^M$ and new data $\mathbf{D}_{\text{new}}$, the importance weights are $w^{(m)} \propto p(\mathbf{D}_{\text{new}} \mid \boldsymbol{\theta}^{(m)})$.  The effective sample size $n_{\text{eff}} = (\sum w^{(m)})^2 / \sum (w^{(m)})^2$ measures how much the new data have changed the posterior.  If $n_{\text{eff}} / M < 0.5$, the reweighted sample is unreliable and full re-estimation is triggered.  This degrades after $\sim$3~years of cumulative updates, at which point Option~C is required.  Option~B (SMC) is the Phase~2 implementation target, as it avoids degeneracy indefinitely but requires more complex implementation.

The following Phase~1 design requirements ensure that sequential updating is retrofittable in Phase~2 without restructuring the MCMC output format:
\begin{enumerate}
  \item \textbf{Full MCMC storage:} Each posterior sample stores the complete parameter vector $\boldsymbol{\theta}^{(m)}$ and the per-observation log-likelihood vector $\ell^{(m)}(t_j)$ for $j = 1, \ldots, N_t$.
  \item \textbf{Modular likelihood:} The likelihood function accepts an arbitrary set of observation times, values, and uncertainties.  Adding a new observation is a single function call that extends the existing observation arrays.
  \item \textbf{Arbitrary-time evaluation:} Component rate models accept any time~$t$, not only the calibration grid, so that predictions can be evaluated at future observation times.
  \item \textbf{Forecast evaluation function:} Given a posterior sample and a new observation, compute the posterior predictive at the observation time, the coverage indicator, and the CRPS (\cref{sec:hindcast}).  This function is the building block for both the hindcast protocol and real-time forecast validation.
\end{enumerate}
...
Glaciers & $a_{\text{gl}} T^2 + b_{\text{gl}} T + c_{\text{gl}}$ & GMST & Melt + area-loss feedback \\
...
\paragraph{Horizon-dependent inadequacy.}  The thermosteric growth function is $g_{\text{th}}(\tau) = 1 - \exp(-\tau / \tau_{\text{th,struct}})$ with $\tau_{\text{th,struct}} \sim 150$~yr.  Growth is slow because thermal expansion physics is well-understood: the thermal expansion coefficient has finite variability, and the main risk is deep-ocean heat uptake efficiency changing under novel forcing.  The exponential saturation reflects the bounded nature of thermosteric model error (\cref{tab:growth-functions}).
...
\paragraph{Model.}  Glacier mass balance responds to both temperature (melt) and precipitation (accumulation).  The temperature sensitivity may be nonlinear because: (a)~small glaciers disappear entirely, reducing the total glacier area; (b)~the elevation--temperature feedback amplifies warming at glacier surfaces.  A model of the form $\text{rate}_{\text{glaciers}}(T) = a_{\text{gl}} \cdot T^2 + b_{\text{gl}} \cdot T + c_{\text{gl}}$ is appropriate, with $a_{\text{gl}} > 0$ reflecting the accelerating loss as glacier area shrinks.

\paragraph{Prior contribution.}  Moderate constraint on $b_{\text{gl}}$ from the geodetic record; weak constraint on $a_{\text{gl}}$ from the relatively short satellite era.  The key question is whether the observed glacier acceleration is separable from ice-sheet acceleration in the GRACE record.

\paragraph{Horizon-dependent inadequacy.}  The glacier growth function is $g_{\text{gl}}(\tau) = \min\!\bigl(1,\, [\tau / \tau_{\text{gl,struct}}]^{1/2}\bigr)$ with $\tau_{\text{gl,struct}} \sim 70$~yr.  The square-root form reflects a random-walk analogy for structural misspecification: the quadratic-in-$T$ model extrapolates poorly in the intermediate regime ($\sim$30--80~yr) where glacier area-loss feedback is nonlinear, but volume-depletion saturation provides a natural ceiling that bounds the structural error (\cref{tab:growth-functions}).
...
\paragraph{Prior contribution.}  The SMB component provides an informed prior on $b_{\text{SMB}}$ from regional climate models (RACMO, MAR); the discharge component contributes through $\beta_{\text{dis}}$ and $c_{\text{dis}}$, constrained by satellite-era velocity and thickness observations.

\paragraph{Horizon-dependent inadequacy.}  The two GrIS sub-components have distinct growth functions.  The SMB growth function is $g_{\text{SMB}}(\tau) = 1 - \exp(-\tau / \tau_{\text{SMB,struct}})$ with $\tau_{\text{SMB,struct}} \sim 100$~yr, reflecting that SMB is well-constrained by temperature with the main risk being nonlinear surface-albedo feedback and firn-aquifer dynamics under sustained warming.  The discharge growth function is $g_{\text{dis}}(\tau) = 1 - \exp(-\tau / \tau_{\text{dis,struct}})$ with $\tau_{\text{dis,struct}} \sim 40$~yr --- faster growth than SMB because discharge dynamics depend on fjord geometry, ice-ocean interaction, and buttressing loss, all of which are poorly constrained by historical data and increasingly important under warming (\cref{tab:growth-functions}).
...
\paragraph{Horizon-dependent inadequacy.}  The AIS component does not use the $g_i(\tau)$ growth-function mechanism.  The A4 deep-uncertainty module already provides horizon-dependent uncertainty through its scenario structure: the within-scenario AIS rate distributions widen with lead time, and the scenario weights shift toward higher-risk scenarios as the forcing departs from the historical range.  During the calibration period, the AIS model-inadequacy scalar $\sigma_{\text{extra,AIS},0}$ reflects the budget-residual uncertainty.  During projection, the A4 module replaces this scalar entirely.  The transition is handled by the forecast regime structure (\cref{sec:forecast-regimes}): in Regime~1, the budget-residual constraint dominates; in Regime~3, the A4 module dominates; in Regime~2, both contribute.
...
\paragraph{Horizon-dependent inadequacy.}  The two TWS sub-components have distinct treatments.  The secular component has $g_{\text{TWS,s}}(\tau) = \tau / \tau_{\text{TWS,struct}}$, reflecting linear growth: the future trajectory of dam construction and groundwater extraction depends on human decisions, and there is no physical bound on how far the future TWS secular rate may deviate from the fitted trend.  The scale is set such that $\sigma_{\text{struct,TWS}} \cdot \sqrt{g(80)} \approx 0.5$~mm/yr, an order-of-magnitude bound on TWS rate uncertainty at 2100.  The climate-driven component has $g_{\text{TWS,c}}(\tau) = 0$ (no growth): the model $\gamma_{\text{TWS}} \cdot E(t)$ is equally valid at all times because the functional form (linear in ENSO index) does not degrade with forecast lead time --- the uncertainty in the TWS climate response lies in the ENSO trajectory itself, which is scenario uncertainty, not model inadequacy (\cref{tab:growth-functions}).

\subsection{Horizon-dependent model averaging (Phase~2)}
\label{sec:horizon-bma}

In Phase~2, multiple model classes may be available for some components (e.g., for glaciers: the polynomial-in-$T$ model, a GP emulator trained on GlacierMIP output, and a PINN surrogate).  The relative skill of these models depends on forecast horizon: the polynomial may be best at short horizons where it is essentially interpolating, while the emulator may be best at long horizons where it captures nonlinear behaviour that the polynomial misses.

Horizon-dependent Bayesian model averaging (BMA) addresses this by assigning model weights that vary with lead time.  The component node interface already supports multiple model classes: any model that provides a rate function $r_i(t; \boldsymbol{\theta}_i)$, a prior $p(\boldsymbol{\theta}_i)$, and a model-inadequacy term $\sigma_{\text{extra},i}(\tau)$ can be registered at a given node.  Multiple model classes at the same node are handled by augmenting the parameter space with a discrete model indicator $M_i \in \{1, \ldots, J_i\}$.  The hindcast protocol (\cref{sec:hindcast}) produces horizon-dependent CRPS for each model class, from which BMA weights at horizon~$\tau$ can be estimated.

Implementation of horizon-dependent BMA is entirely Phase~2.  The Phase~1 requirement is limited to verifying that the node interface can accept multiple model classes without modification.
...
The full hierarchical model has the following structure.  Let $\boldsymbol{\theta}_i$ denote the parameter vector for component~$i$ (with component-specific dimension and interpretation; see~\cref{tab:component-models}), and let $\boldsymbol{\theta}_{\text{global}} = (a, b, c, \tau, H_0, \sigma_{\text{extra}})$ denote the Level~1 global parameters.  The joint posterior is:
\begin{align}
  &p(\boldsymbol{\theta}_{\text{global}}, \boldsymbol{\theta}_1, \ldots, \boldsymbol{\theta}_K, \boldsymbol{\eta} \mid \mathbf{H}_{\text{obs}}, \mathbf{D}_1, \ldots, \mathbf{D}_K) \nonumber \\
  &\quad \propto
  \underbrace{p(\mathbf{H}_{\text{obs}} \mid \boldsymbol{\theta}_{\text{global}}, \boldsymbol{\eta})}_{\text{GMSL likelihood}}
  \cdot \underbrace{\prod_{j=1}^{N_t} p_{\text{budget}}\!\left(\text{rate}_{\text{total}}(t_j) - \sum_{i=1}^{K} \text{rate}_i(t_j;\, \boldsymbol{\theta}_i) \;\middle|\; \sigma_{\text{budget}}(t_j)\right)}_{\text{rate-level budget constraint}} \nonumber \\
  &\qquad \cdot \prod_{i=1}^{K} \underbrace{p(\mathbf{D}_i \mid \boldsymbol{\theta}_i)}_{\text{component likelihoods}}
...
\end{align}
where $p_{\text{budget}}$ is the Gaussian soft constraint from~\cref{eq:budget-constraint}, $\boldsymbol{\eta}$ collects nuisance parameters, and $\mathbf{D}_i$ is the component-specific dataset.  The budget tolerance $\sigma_{\text{budget}}(t_j)$ is given by~\cref{eq:budget-tolerance}.
...
The total GMSL projection $H(t)$ depends on three sources of uncertainty: the SSP scenario index~$S$ (discrete), the A4 deep-uncertainty scenario index~$k$ (discrete), and the Level~1 + Level~2 parameters~$\boldsymbol{\theta}$ (continuous).  The total variance is given by iterated application of the law of total variance:
\begin{equation}
  \text{Var}[H(t)]
...
In practice, $C_{\text{cross}}(t)$ is computed directly from the MCMC posterior samples: at each draw~$\boldsymbol{\theta}^{(m)}$, evaluate all component rates and compute the sample covariance across draws.  No analytic approximation is required.

\subsection{Horizon-dependent model inadequacy}
\label{sec:horizon-inadequacy}

The component-level model inadequacy terms $\sigma_{\text{extra},i}$ introduced in~\cref{eq:budget-tolerance} are calibrated during the historical period and, absent correction, are applied unchanged during the forecast period.  This is physically wrong: a model calibrated on 1900--2018 data has monotonically increasing inadequacy as it forecasts further from the calibration window, for two reasons.  First, under all SSP scenarios, GMST exceeds the calibration range by $\sim$2040--2050, so component models operate in extrapolation with structural error that grows as the forcing departs from the observed range.  Second, nonlinear feedbacks (MISI, glacier area-loss cascading, AMOC changes affecting thermosteric response) may activate at longer horizons; these processes are not captured by the calibrated parametric models and represent structural inadequacy that grows with lead time.

The scalar $\sigma_{\text{extra},i}$ therefore produces overconfident forecasts at long horizons and slightly underconfident forecasts at very short horizons (where the system trajectory is tightly constrained and the parametric model is essentially interpolating).

\paragraph{Horizon-dependent formulation.}
Replace the scalar $\sigma_{\text{extra},i}$ with a horizon-dependent function $\sigma_{\text{extra},i}(\tau)$, where $\tau = t - t_{\text{cal,end}}$ is the forecast lead time.  During the calibration period ($\tau \leq 0$), the model inadequacy reduces to the calibration scalar:
\begin{equation}
  \sigma_{\text{extra},i}(\tau) = \sigma_{\text{extra},i,0} \quad \text{for } \tau \leq 0
\end{equation}
During the forecast period ($\tau > 0$), it grows:
\begin{equation}
  \sigma_{\text{extra},i}^2(\tau) = \sigma_{\text{extra},i,0}^2 + g_i(\tau) \cdot \sigma_{\text{struct},i}^2
  \label{eq:sigma-extra-horizon}
\end{equation}
where $\sigma_{\text{extra},i,0}$ is the calibration-period model inadequacy (identical to the current $\sigma_{\text{extra},i}$; learned from data via a HalfCauchy prior), $\sigma_{\text{struct},i}$ is a structural uncertainty scale representing the maximum additional model inadequacy at long lead times (a new parameter with a component-specific HalfCauchy prior), and $g_i(\tau)$ is a dimensionless growth function satisfying $g_i(0) = 0$ and $g_i(\tau) \to 1$ as $\tau \to \infty$, with shape determined by the physics of each component.

The total predictive model-inadequacy variance at forecast lead time $\tau$ is:
\begin{equation}
  \sigma_{\text{pred}}^2(\tau) = \sum_{i=1}^{K} \sigma_{\text{extra},i}^2(\tau)
  \label{eq:sigma-pred}
\end{equation}

\paragraph{Component-specific growth functions.}
The functional form of $g_i(\tau)$ encodes how model inadequacy accumulates for each component.  \Cref{tab:growth-functions} specifies the growth functions and their physical rationale.  The key distinctions are: (a)~exponential saturation for components whose physics is well-understood and whose model error is bounded (thermosteric, SMB); (b)~square-root growth for components whose structural error accumulates diffusively (glaciers); (c)~linear growth for components driven by human decisions with no physical bound on future trajectory (TWS secular); (d)~no growth for components where the functional form is equally valid at all times (TWS climate); (e)~not directly applicable for AIS, where the A4 module provides its own horizon-dependent uncertainty.

\begin{table}[ht]
\centering
\caption{Component-specific growth functions $g_i(\tau)$ for the horizon-dependent model inadequacy.  The characteristic timescale $\tau_{i,\text{struct}}$ governs the rate of growth; $\sigma_{\text{struct},i}$ is the asymptotic structural uncertainty scale.  In Phase~1, $\tau_{i,\text{struct}}$ is fixed at the prior median; $\sigma_{\text{struct},i}$ is drawn from the prior with no likelihood update.}
\label{tab:growth-functions}
\small
\begin{tabular}{lp{4.2cm}p{5.5cm}l}
\toprule
Component & $g_i(\tau)$ & Rationale & $\tau_{i,\text{struct}}$ prior \\
\midrule
Thermosteric & $1 - \exp(-\tau/\tau_{\text{th,struct}})$ & Slow growth; physics well-understood; bounded thermal expansion coefficient variability. & LogN$(\ln 150, 0.5)$ \\
\addlinespace
Glaciers & $\min\!\bigl(1,\, [\tau/\tau_{\text{gl,struct}}]^{1/2}\bigr)$ & Moderate growth; area-loss feedback nonlinear in intermediate regime; volume depletion provides natural ceiling. & LogN$(\ln 70, 0.5)$ \\
\addlinespace
GrIS (SMB) & $1 - \exp(-\tau/\tau_{\text{SMB,struct}})$ & Similar to thermosteric; well-constrained by temperature; risk is nonlinear albedo feedback. & LogN$(\ln 100, 0.5)$ \\
\addlinespace
GrIS (discharge) & $1 - \exp(-\tau/\tau_{\text{dis,struct}})$ & Faster growth; discharge depends on fjord geometry and ice-ocean interaction, poorly constrained by historical data. & LogN$(\ln 40, 0.5)$ \\
\addlinespace
AIS & \multicolumn{3}{l}{Not applicable --- A4 deep-uncertainty module provides horizon-dependent uncertainty during projection.} \\
\addlinespace
TWS (secular) & $\tau / \tau_{\text{TWS,struct}}$ & Linear growth; anthropogenic parameter trajectory (dams, groundwater) depends on human decisions with no physical bound. & Set s.t.\ $\sigma_{\text{struct,TWS}} \sqrt{g(80)} \approx 0.5$~mm/yr \\
\addlinespace
TWS (climate) & $0$ (no growth) & Model $\gamma_{\text{TWS}} \cdot E(t)$ is equally valid at all times; uncertainty is in the ENSO trajectory, not model form. & N/A \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Note on AIS.}  The AIS component does not use a parametric forward model during projection; the A4 module provides its own horizon-dependent uncertainty through its scenario structure.  The growth function $g_i(\tau)$ applies only to components with parametric rate models.  During the calibration period, the AIS component has a $\sigma_{\text{extra,AIS},0}$ that reflects the budget-residual model inadequacy; during projection, the A4 module replaces this entirely.  The transition between these regimes is handled by the forecast regime structure (\cref{sec:forecast-regimes}).

\paragraph{Budget tolerance during the forecast period.}
The budget tolerance (\cref{eq:budget-tolerance}) is modified during the forecast period.  The observational uncertainty term $\sigma_{\text{obs,total}}^2(t_j)$ vanishes because there are no observations to constrain, and the component model-inadequacy terms become horizon-dependent:
\begin{equation}
  \sigma_{\text{budget,forecast}}^2(t) = \sum_{i=1}^{K} \sigma_{\text{extra},i}^2(\tau), \qquad \tau = t - t_{\text{cal,end}}
  \label{eq:budget-tolerance-forecast}
\end{equation}
This means the budget constraint is tighter at short forecast horizons (where model inadequacy is small, so component rates must sum closely) and progressively looser at long horizons (where model inadequacy is large, so more budget imbalance is tolerated).  This is physically correct: at short horizons, the component decomposition is well-constrained; at long horizons, it is increasingly uncertain.

\paragraph{New parameters and Phase~1 treatment.}
The horizon-dependent formulation adds, per applicable component, two new parameters: $\sigma_{\text{struct},i}$ (HalfCauchy prior, component-specific scale) and $\tau_{i,\text{struct}}$ (LogNormal prior, component-specific; see~\cref{tab:growth-functions}).  These parameters are not identifiable from the calibration data alone.  In Phase~1, $\tau_{i,\text{struct}}$ is fixed at the prior median; $\sigma_{\text{struct},i}$ is drawn from the prior with no likelihood update.  This makes the horizon-dependent inadequacy a \emph{prior predictive} feature: it widens the forecast uncertainty band relative to the scalar model, consistent with the physical expectation that model error grows with extrapolation distance, without requiring additional data to calibrate.  In Phase~2, the hindcast cross-validation (\cref{sec:hindcast}) will provide empirical constraints on both the growth-function form and the structural uncertainty scale.
...
\subsection{Forecast regime structure}
\label{sec:forecast-regimes}

The variance decomposition reveals a natural partition of the forecast period into three regimes, distinguished by which uncertainty source dominates and by the appropriate credibility basis for the projections.  The regime boundaries are not free parameters --- they are emergent properties of the posterior.

\paragraph{Regime~1: Observational constraint dominant ($\tau < \tau_{\text{obs}}$).}
At short lead times ($\tau \approx 0$--15~years), the system trajectory is strongly constrained by its current state, recent acceleration, and near-term committed warming.  The dominant uncertainty sources are interannual variability (aleatoric; ENSO, volcanic), near-term forcing uncertainty (small; the SSP trajectories are effectively indistinguishable for $\sim$15~years), and parameter uncertainty in the rate--temperature relationship.  Process models play a minimal role: they carry substantial initialisation errors and are outperformed by observation-constrained statistical models at these horizons.  The deep-uncertainty module (A4) is negligible: the AIS contribution is small and well-constrained by the budget residual over the next decade.  The credibility basis for Regime~1 projections is hindcast cross-validation (\cref{sec:hindcast}): the framework must demonstrate forecast skill exceeding simple trend extrapolation at all horizons within this regime.

\paragraph{Regime~2: Physics-informed projection ($\tau_{\text{obs}} < \tau < \tau_{\text{diverge}}$).}
At intermediate horizons ($\tau \approx 15$--60~years), the forcing scenarios begin to diverge materially, and the component models extrapolate beyond their calibration domain.  The dominant uncertainty sources are scenario uncertainty (growing), structural/model-form uncertainty (growing; captured by the horizon-dependent model inadequacy of~\cref{sec:horizon-inadequacy}), and WAIS dynamics (emerging; possible early-stage MISI).  The credibility basis is physical plausibility of the component models, consistency with the Regime~1 posterior at the transition point, budget closure, and process-model structural information informing priors.  The hindcast validation provides partial coverage for the early part of this regime.  The A4 module becomes the primary uncertainty source for the AIS contribution as the budget-residual constraint weakens.

\paragraph{Regime~3: Deep uncertainty dominant ($\tau > \tau_{\text{diverge}}$).}
At long horizons ($\tau \approx 60$--200~years), the projection envelope is dominated by structural and scenario uncertainty.  For well-behaved components (thermosteric, glaciers, GrIS), scenario uncertainty is the leading term.  For AIS, the A4 deep-uncertainty module is dominant, with the ice-sheet contribution being the primary source of high-end risk.  The credibility basis shifts from empirical validation to physical constraints (conservation laws, reservoir limits, paleoclimate analogues), internal consistency (budget closure, process understanding), and expert assessment (A4 scenario weights).  There is no direct empirical validation at these horizons for the specific forcing scenarios of interest.

\paragraph{Transition timescales.}
The boundaries $\tau_{\text{obs}}$ and $\tau_{\text{diverge}}$ are approximate and scenario-dependent.  Under SSP1-2.6, Regime~2 extends further because the system stays closer to the calibration domain; under SSP5-8.5, Regime~3 begins earlier because the system enters novel territory sooner.  Formally:
\begin{itemize}
  \item $\tau_{\text{obs}}$ is the \emph{skill horizon} from the hindcast cross-validation (\cref{sec:hindcast}): the lead time at which the framework's forecast skill degrades below simple extrapolation.
  \item $\tau_{\text{diverge}}$ is the horizon at which the across-scenario variance $\text{Var}_S[E_{\boldsymbol{\theta},k}[H(t) \mid S]]$ exceeds the within-scenario uncertainty (parameter + structural + A4) for the majority of components, computed from the variance decomposition (\cref{eq:variance-outer,eq:variance-inner}).
\end{itemize}

These emergent timescales depend on the posterior and on the scenario set; they should be reported as scenario-conditional ranges rather than single numbers.  The three-regime framing is a conceptual and communicative tool --- not a mathematical partition.  All three uncertainty sources contribute at all horizons; the regime labels identify which source is \emph{dominant}.  \Cref{tab:forecast-regimes} summarises the regime structure.

\begin{table}[ht]
\centering
\caption{Summary of the three forecast regimes.  The regime boundaries are emergent properties of the posterior, not preset values; approximate horizons are given for orientation.}
\label{tab:forecast-regimes}
\small
\begin{tabular}{p{3.2cm}p{3.5cm}p{3.8cm}p{3.5cm}}
\toprule
Property & Regime~1 ($\tau < \tau_{\text{obs}}$) & Regime~2 ($\tau_{\text{obs}} < \tau < \tau_{\text{diverge}}$) & Regime~3 ($\tau > \tau_{\text{diverge}}$) \\
\midrule
Approx.\ horizon & 0--15~yr & 15--60~yr & 60--200~yr \\
\addlinespace
Dominant methodology & Rate-and-state model; observation-constrained & Component parametric/emulator models; physics-informed & Process-model-informed; A4 deep-uncertainty \\
\addlinespace
Dominant uncertainty & Parameter + aleatoric & Scenario + structural & Deep uncertainty + scenario \\
\addlinespace
Credibility basis & Hindcast cross-validation & Physical plausibility; budget closure & Conservation laws; paleo constraints; expert assessment \\
\addlinespace
A4 role & Negligible & Growing & Dominant (AIS) \\
\addlinespace
Model inadequacy & $\sigma_{\text{extra},i,0}$ (calibration value) & $g_i(\tau)$ growing & $g_i(\tau) \to 1$; saturated \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Horizon-stratified uncertainty composition}
\label{sec:horizon-reporting}

The variance decomposition (\cref{eq:variance-outer,eq:variance-inner}) combined with the horizon-dependent model inadequacy (\cref{eq:sigma-extra-horizon}) produces an uncertainty composition that changes qualitatively with forecast horizon.  This is one of the most communicable results of the framework: it directly answers the question that decision-makers require --- how confident should projections at 2050 vs.\ 2100 vs.\ 2150 be, and why.

\paragraph{Reporting horizons.}  The variance decomposition is evaluated at $t \in \{2030, 2050, 2075, 2100, 2150, 2200\}$ for each SSP scenario.  These horizons span all three forecast regimes.

\paragraph{Reported quantities.}  At each reporting horizon~$t$ and scenario~$S$:
\begin{enumerate}
  \item Total projection variance $\text{Var}[H(t) \mid S]$.
  \item Fractional contributions from each uncertainty source:
    \begin{align}
      f_{\text{param}}(t, S) &= \frac{E_k\!\left[\text{Var}_{\boldsymbol{\theta}}[H(t) \mid S, k]\right]}{\text{Var}[H(t) \mid S]} &\quad &\text{(parameter uncertainty)} \\
      f_{\text{A4}}(t, S) &= \frac{\text{Var}_k\!\left[E_{\boldsymbol{\theta}}[H(t) \mid S, k]\right]}{\text{Var}[H(t) \mid S]} &\quad &\text{(A4 deep uncertainty)} \\
      f_{\text{struct}}(t, S) &= \frac{\sum_i g_i(\tau) \cdot \sigma_{\text{struct},i}^2}{\text{Var}[H(t) \mid S]} &\quad &\text{(model inadequacy)} \\
      f_{\text{scenario}}(t) &= \frac{\text{Var}_S\!\left[E_{\boldsymbol{\theta},k}[H(t) \mid S]\right]}{\text{Var}[H(t)]} &\quad &\text{(scenario spread, across SSPs)}
    \end{align}
  \item Component-level fractional contributions (which physical component dominates the total variance at each horizon).
  \item The forecast regime (1, 2, or~3) for each component at each horizon, based on the transition timescales from~\cref{sec:forecast-regimes}.
\end{enumerate}

\paragraph{Presentation.}  \Cref{fig:variance-horizon} will present the horizon-stratified decomposition as a two-panel summary figure.  Panel~A shows fractional contributions of parameter, scenario, A4, and model-inadequacy uncertainty at each reporting horizon for a reference scenario (SSP2-4.5).  Panel~B shows fractional contributions of each physical component (thermosteric, glaciers, GrIS, AIS, TWS) to the total variance at each reporting horizon.  Contributions that are prior-dominated (as opposed to data-constrained) should be visually distinguished (e.g., hatched shading) to communicate which parts of the decomposition are empirically grounded and which are informed by physical reasoning alone.

\begin{figure}[ht]
\centering
\fbox{\parbox{0.9\textwidth}{\centering\vspace{3cm}
  \textbf{Placeholder: Horizon-stratified variance decomposition}\\[6pt]
  \emph{Panel~A:} Stacked bar chart of uncertainty-source fractions ($f_{\text{param}}$, $f_{\text{A4}}$, $f_{\text{struct}}$, $f_{\text{scenario}}$) at each reporting horizon for SSP2-4.5.  Hatched shading for prior-dominated contributions.\\[6pt]
  \emph{Panel~B:} Stacked bar chart of component fractions (thermosteric, glaciers, GrIS, AIS, TWS) at each reporting horizon.\\[6pt]
  \emph{To be populated when the Phase~1 posterior is available.}
\vspace{3cm}}}
\caption{Horizon-stratified variance decomposition.  The composition of total projection uncertainty changes qualitatively with forecast horizon, transitioning from parameter-dominated at short horizons through scenario-dominated at intermediate horizons to deep-uncertainty-dominated at long horizons (\cref{sec:forecast-regimes}).  Results for two SSP-weighting schemes (uniform and plausibility-weighted) should be compared, as the scenario-uncertainty fraction is sensitive to this choice.}
\label{fig:variance-horizon}
\end{figure}

\paragraph{Sensitivity to scenario weighting.}  The fractional decomposition depends on the weights assigned to the SSP scenarios.  Under uniform weighting, scenario uncertainty will dominate at most horizons; under plausibility-weighted schemes (e.g., downweighting SSP5-8.5), the decomposition shifts toward parameter and structural uncertainty.  Results should be reported for at least two weighting schemes to make this sensitivity visible.


% ====================================================================
\section{Forecast Skill Validation}
\label{sec:hindcast}
% ====================================================================

The framework includes no mechanism for evaluating its own forecast skill as a function of lead time unless one is explicitly specified.  Without such a mechanism, there is no empirical basis for calibrating the growth functions $g_i(\tau)$ in~\cref{sec:horizon-inadequacy}, no demonstration that the framework produces reliable short-term forecasts, and no basis for comparing the framework's skill to simpler alternatives.  This section defines a hindcast cross-validation protocol as a mandatory diagnostic.

\subsection{Hindcast protocol}

For each training cutoff year $Y \in \mathcal{Y} = \{1960, 1970, 1980, 1990, 2000\}$:
\begin{enumerate}
  \item Fit the full joint model (\cref{eq:joint-posterior}) using only data up to year~$Y$: GMSL observations $\mathbf{H}_{\text{obs}}(t)$ for $t \leq Y$; component datasets $\mathbf{D}_i(t)$ for $t \leq Y$ (where available --- for cutoff years before the satellite era, some component datasets are empty).
  \item Generate posterior predictive distributions for $H(t)$ at forecast horizons $\tau \in \{10, 20, 30, 40, 50\}$~years, where $t = Y + \tau \leq 2018$ so that held-out observations are available for comparison.
  \item Compute forecast skill metrics by comparing the posterior predictive to the held-out observations $\mathbf{H}_{\text{obs}}(Y + \tau)$.
\end{enumerate}

\subsection{Skill metrics}

For each cutoff year~$Y$ and forecast horizon~$\tau$:

\paragraph{Coverage probability.}  The fraction of held-out observations falling within the $p$-percent posterior predictive interval, evaluated at $p \in \{50, 90, 95\}$:
\begin{equation}
  \text{Coverage}_p(\tau) = \frac{1}{|\mathcal{Y}|} \sum_{Y \in \mathcal{Y}} \mathbf{1}\!\left(H_{\text{obs}}(Y+\tau) \in \text{CI}_p(Y, \tau)\right)
  \label{eq:coverage}
\end{equation}
where $\text{CI}_p(Y, \tau)$ is the $p$-percent posterior predictive interval from the model trained on data up to year~$Y$.  Overconfidence manifests as coverage below the nominal rate; underconfidence as coverage above.

\paragraph{Continuous Ranked Probability Score (CRPS).}  The CRPS measures the full distributional forecast quality:
\begin{equation}
  \text{CRPS}(F_{Y,\tau},\, H_{\text{obs}}(Y + \tau)) = \int_{-\infty}^{\infty} \left[F_{Y,\tau}(x) - \mathbf{1}(x \geq H_{\text{obs}}(Y + \tau))\right]^2 \, \mathrm{d}x
  \label{eq:crps}
\end{equation}
where $F_{Y,\tau}$ is the posterior predictive CDF from the model trained on data up to year~$Y$, evaluated at horizon~$\tau$.  Lower CRPS indicates better distributional calibration.  The CRPS is a strictly proper scoring rule: it is minimised in expectation only by the true data-generating distribution.

\paragraph{Interval width.}  The width of the 90\% posterior predictive interval at each horizon.  What matters is not the width per se but whether the interval width grows with $\tau$ in a way that is consistent with the coverage remaining near nominal.

\paragraph{Skill relative to baseline.}  The framework is compared to two reference forecasts:
\begin{itemize}
  \item \emph{Linear trend extrapolation:} Fit a linear trend to $\mathbf{H}_{\text{obs}}(t)$ for $t \leq Y$ and extrapolate with Gaussian prediction interval.
  \item \emph{Climatological drift:} Project the rate observed at year~$Y$ forward with no change, with prediction interval widening at $\sigma_{\text{rate}} \sqrt{\tau}$.
\end{itemize}
The framework should demonstrate positive skill (lower CRPS) relative to both baselines at all horizons up to the skill horizon $\tau^*$.

\subsection{Skill horizon}

The \emph{skill horizon} $\tau^*$ is the forecast lead time at which the framework's CRPS degrades to the level of the linear-trend baseline.  Beyond $\tau^*$, the parametric model adds no value over simple extrapolation, and the forecast should be dominated by the structural uncertainty terms $\sigma_{\text{struct},i}$ and, for AIS, by the A4 deep-uncertainty module.

The skill horizon is component-specific and forcing-scenario-dependent: models extrapolate better under mild scenarios that stay closer to the calibration range.  Report $\tau^*_i$ for each component and scenario.  The skill horizon $\tau^*$ for the total GMSL serves as the empirical estimate of $\tau_{\text{obs}}$, the Regime~1/Regime~2 transition timescale in~\cref{sec:forecast-regimes}.

\subsection{Calibrating the growth functions from hindcast data}

If the framework is overconfident at horizon~$\tau$ (coverage below nominal for multiple cutoff years), the growth functions $g_i(\tau)$ are too slow --- the model inadequacy $\sigma_{\text{extra},i}(\tau)$ is too small.  Conversely, if the framework is underconfident, the growth functions are too aggressive.  The hindcast residuals provide an empirical constraint:
\begin{equation}
  g_i(\tau) \approx \frac{\hat{\sigma}_{\text{resid},i}^2(\tau) - \sigma_{\text{extra},i,0}^2}{\sigma_{\text{struct},i}^2}
\end{equation}
where $\hat{\sigma}_{\text{resid},i}^2(\tau)$ is the empirical mean squared forecast error at horizon~$\tau$, estimated across cutoff years.  In practice, the empirical signal is noisy (only 5 cutoff years, each with limited hold-out data), so a hybrid approach --- using the physics-informed functional form from~\cref{tab:growth-functions} but calibrating its parameters from hindcast data --- is likely optimal.  This calibration is deferred to Phase~2; in Phase~1, the growth functions are set by the physics-informed priors alone.

\paragraph{Open limitations.}  With only 5 cutoff years, the coverage estimates have substantial sampling noise: coverage of 3/5 vs.\ 4/5 vs.\ 5/5 at the 90\% level is not statistically distinguishable at conventional significance levels.  The diagnostic is informative as a pattern across horizons and cutoffs, not as a point estimate.  The hindcast protocol requires re-running the MCMC for each cutoff year, which is computationally feasible for Phase~1 (polynomial rate models, $\sim$minutes per run) but may be substantial for Phase~2 (emulator-based models).  Most importantly, the hindcast does not test the framework's ability to forecast under forcing regimes that have not been observed: all held-out data are within the historical forcing envelope.  The hindcast validates interpolation skill, not extrapolation skill.
...
\addlinespace
\multicolumn{4}{l}{\textit{Cross-cutting capabilities}} \\
\addlinespace
-- & Hindcast cross-validation & Held-out GMSL observations & Phase~1 \\
-- & Sequential update interface (design) & -- & Phase~1 \\
-- & Sequential update (SMC implementation) & Annual observation updates & Phase~2 \\
-- & Horizon-dependent model averaging & Multiple model classes & Phase~2 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}
  ...
  \item \textbf{Multi-timescale forecasting:} The framework explicitly recognises that the dominant uncertainty source and the appropriate credibility basis change with forecast horizon (\cref{sec:forecast-regimes}).  Horizon-dependent model inadequacy (\cref{sec:horizon-inadequacy}) prevents overconfident long-range forecasts, and the hindcast cross-validation (\cref{sec:hindcast}) provides empirical validation of short-range forecast skill.  The horizon-stratified variance decomposition (\cref{sec:horizon-reporting}) makes the framework's own credibility as a function of lead time transparent and quantifiable.
\end{enumerate}

The framework is designed not to produce a single ``best estimate'' of future SLR, but to provide a \emph{defensible probabilistic envelope} that honestly reflects what is known, what is uncertain, and what is deeply uncertain --- and to do so in a way that is formally self-consistent across scales, components, time horizons, and methodological approaches.
