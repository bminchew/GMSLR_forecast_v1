...
\end{itemize}
Each scenario has its own within-scenario distribution; the mixture weights encode expert assessment of scenario plausibility.  The resulting deep-uncertainty contribution enters the total projection variance through the nested law-of-total-variance decomposition (\cref{sec:variance}).  The approximation that the A4 contribution is independent of the Level~1 calibration uncertainty (Assumption~V1 in~\cref{sec:variance-approx}) requires that the A4 scenarios be consistent with the historical budget residual; this condition is tested by the A4/budget consistency diagnostic (\cref{sec:a4-consistency}).
...
Scenario weights & $w_k$ & D & Prior mixture weights from expert assessment; updated by budget consistency (\cref{sec:a4-consistency}). \\
\addlinespace
...
\subsection{A4/budget consistency}
\label{sec:a4-consistency}

The A4 deep-uncertainty module (\cref{sec:ais}) assigns prior mixture weights $w_k^{\text{prior}}$ to four AIS scenarios (S1--S4) on the basis of expert assessment and process understanding.  These weights are calibrated against physical plausibility and future expert judgment, not against the observational record.  But each scenario also implies a historical AIS rate trajectory during the calibration period, because the temporal scaling functions $g_k(t)$ (quadratic accumulation for S1; delayed-exponential ramp-up for S2--S4) are defined back to the scenario onset times.  The budget-derived AIS rate provides an observationally grounded check on this implication.

\subsubsection{Budget-derived AIS rate}

During the calibration period, the AIS rate is obtained as a budget residual:
\begin{equation}
  r_{\text{AIS}}^{\text{budget}}(t) = r_{\text{total}}(t) - \sum_{i \neq \text{AIS}} r_i(t;\, \boldsymbol{\theta}_i)
  \label{eq:ais-budget-residual}
\end{equation}
This quantity is not a direct observation of the AIS rate.  It inherits the uncertainties and biases of every non-AIS component model.  Its uncertainty is:
\begin{equation}
  \sigma_{\text{res}}^2(t) = \sigma_{\text{obs,total}}^2(t) + \sum_{i \neq \text{AIS}} \left[\sigma_{D_i}^2(t) + \sigma_{\text{extra},i}^2\right]
  \label{eq:sigma-res}
\end{equation}
where $\sigma_{\text{obs,total}}^2(t)$ is the GMSL observation uncertainty, $\sigma_{D_i}^2(t)$ the component observation uncertainty, and $\sigma_{\text{extra},i}^2$ the component model-inadequacy term (see~\cref{eq:budget-tolerance}; full treatment deferred to Item~5 of the revision plan).  The budget-derived AIS rate is therefore a joint function of all other components' posterior distributions, not a clean observational constraint.

\emph{Pre-GRACE caveat.}  Before $\sim$2002, no direct constraint on the AIS rate exists from GRACE or altimetry.  The budget residual is the sole source of AIS information, and $\sigma_{\text{res}}(t)$ is correspondingly large.  During this interval, any systematic bias in the non-AIS component models (e.g., thermosteric overestimate, glacier underestimate) propagates directly into $r_{\text{AIS}}^{\text{budget}}(t)$.  The consistency diagnostic below is therefore informative only during the post-2002 interval where component-specific observations partially decorrelate the residual from individual component biases.

\subsubsection{Historical consistency likelihood}

For each A4 scenario $k$, the temporal scaling function $g_k(t)$ defines an implied AIS rate trajectory $r_{\text{AIS}}^{(k)}(t)$ during the calibration period.  The consistency of this trajectory with the budget-derived AIS rate is measured by:
\begin{equation}
  p\!\left(r_{\text{AIS}}^{\text{budget}} \mid k\right)
    = \prod_{t_j \in \mathcal{T}_{\text{overlap}}} \mathcal{N}\!\left(r_{\text{AIS}}^{\text{budget}}(t_j) \;\middle|\; r_{\text{AIS}}^{(k)}(t_j),\; \sigma_{\text{res}}^2(t_j)\right)
  \label{eq:consistency-likelihood}
\end{equation}
where $\mathcal{T}_{\text{overlap}}$ is the set of calibration times during which the A4 scenarios produce distinguishable rates.  In the current A4 specification, S1 (status quo) diverges from S2--S4 only after $\sim$2013, so $\mathcal{T}_{\text{overlap}} \subseteq \{2013, 2014, \ldots, 2018\}$.  This is a short window --- six annual observations at most --- and the scenario-implied rates are close together relative to $\sigma_{\text{res}}$ during this interval.  The consistency likelihood is therefore expected to be nearly flat across scenarios in Phase~1, with $p(r_{\text{AIS}}^{\text{budget}} \mid k)$ varying by at most a factor of $\sim$2--3 across $k$.

Two points require emphasis.  First, $\sigma_{\text{res}}(t_j)$ in~\cref{eq:consistency-likelihood} must reflect the \emph{total} uncertainty on the budget residual --- propagated from all non-AIS component uncertainties and the GMSL observation error --- not merely the IMBIE measurement error on AIS.  Underestimating $\sigma_{\text{res}}$ produces spuriously confident scenario discrimination.  Second, $\sigma_{\text{res}}(t_j)$ is not a fixed quantity: it depends on the component model-inadequacy parameters $\sigma_{\text{extra},i}$, which are sampled during MCMC.  The consistency likelihood must therefore be evaluated at each posterior draw, using the draw-specific $\sigma_{\text{res}}(t_j)$.

\subsubsection{Posterior scenario weights}

The prior weights are updated by Bayes' rule:
\begin{equation}
  w_k^{\text{post}} = \frac{w_k^{\text{prior}} \cdot p\!\left(r_{\text{AIS}}^{\text{budget}} \mid k\right)}{\sum_{j=1}^{4} w_j^{\text{prior}} \cdot p\!\left(r_{\text{AIS}}^{\text{budget}} \mid j\right)}
  \label{eq:posterior-weights}
\end{equation}
with prior weights $w_k^{\text{prior}}$ from the A4 module (S1: 0.10, S2: 0.55, S3: 0.25, S4: 0.10; see companion document).  Because $r_{\text{AIS}}^{\text{budget}}$ is itself a posterior quantity (it depends on the sampled component parameters $\boldsymbol{\theta}_i$), the posterior weights $w_k^{\text{post}}$ are random variables.  The diagnostic reports the posterior distribution of $w_k^{\text{post}}$ across MCMC draws, not a single point estimate.

\subsubsection{Phase~1 implementation: post-hoc diagnostic}

In Phase~1, the A4 scenario index $k$ is \emph{not} sampled during the main MCMC.  The consistency diagnostic is computed post hoc:
\begin{enumerate}
  \item Run the main MCMC over all component parameters $\boldsymbol{\theta}_1, \ldots, \boldsymbol{\theta}_K$ and global parameters $\boldsymbol{\theta}_{\text{global}}$, without the A4 discrete index.
  \item For each posterior draw $m = 1, \ldots, M$, compute the budget-derived AIS rate $r_{\text{AIS}}^{\text{budget},(m)}(t_j)$ at each overlap time $t_j$ and the draw-specific residual uncertainty $\sigma_{\text{res}}^{(m)}(t_j)$.
  \item Evaluate the consistency likelihood $p(r_{\text{AIS}}^{\text{budget},(m)} \mid k)$ for each scenario $k$ using~\cref{eq:consistency-likelihood}.
  \item Compute the posterior weights $w_k^{\text{post},(m)}$ using~\cref{eq:posterior-weights}.
  \item Report: (a)~the draw-averaged posterior weights $\bar{w}_k^{\text{post}} = M^{-1}\sum_m w_k^{\text{post},(m)}$; (b)~the posterior standard deviation of $w_k^{\text{post}}$ across draws; (c)~the Kullback--Leibler divergence $D_{\text{KL}}(w^{\text{post}} \| w^{\text{prior}})$ as a scalar summary of how much the budget residual has updated the expert assessment.
\end{enumerate}

If $D_{\text{KL}}(w^{\text{post}} \| w^{\text{prior}}) < 0.01$~nats, the observational record has negligible discriminatory power and the Phase~1 decision to defer full joint inference is validated.  If $D_{\text{KL}} > 0.1$~nats, the budget residual materially updates the scenario weights, and the Phase~2 joint inference should be prioritised.

The rationale for deferring joint inference in Phase~1 is computational, not conceptual.  Sampling the discrete index $k$ at each MCMC step requires either a Gibbs update over the four-valued categorical (straightforward but requires mixing diagnostics for the discrete variable) or reversible-jump MCMC (more general but harder to tune).  Given that the expected information gain from the short overlap window is negligible, the implementation cost is not justified by the scientific return in Phase~1.

\subsubsection{Phase~2: full joint inference}

When the observational record extends sufficiently that $D_{\text{KL}}(w^{\text{post}} \| w^{\text{prior}}) > 0.1$~nats in the post-hoc diagnostic, or when extended GRACE-FO data are incorporated, the scenario index $k$ is embedded as a discrete latent variable in the MCMC.  At each iteration:
\begin{enumerate}
  \item Propose component parameters $\boldsymbol{\theta}_i$ for all components (continuous block).
  \item Compute the budget residual $r_{\text{AIS}}^{\text{budget}}(t_j)$ at each calibration time.
  \item For each A4 scenario $k$, evaluate $p(r_{\text{AIS}}^{\text{budget}} \mid k)$ using~\cref{eq:consistency-likelihood}.
  \item Sample $k$ from the categorical distribution $\text{Cat}(w_1^{\text{post}}, \ldots, w_4^{\text{post}})$ with weights given by~\cref{eq:posterior-weights}.
  \item Conditional on $k$, sample the within-scenario AIS parameters.
\end{enumerate}
This is a standard data augmentation scheme for finite mixture models \citep{gelman2013bayesian}.  The categorical update is a Gibbs step conditioned on the current values of all continuous parameters.

\subsubsection{Cross-validation with IMBIE}

The budget-derived AIS rate should be cross-validated against direct IMBIE Antarctica observations during the GRACE era (2002--present).  Systematic discrepancies indicate either errors in the component decomposition (if the budget residual is biased relative to IMBIE) or inconsistencies in the GRACE-derived products (if the component-specific GRACE estimates are incompatible with IMBIE when summed).  This validation is specified in the component decomposition plan (Stage~4) and provides an independent check on the quality of the budget residual that feeds into the consistency diagnostic.

\subsubsection{Design constraint for paleo extensions}

The consistency likelihood (\cref{eq:consistency-likelihood}) must accept arbitrary time windows, not only the modern calibration period.  Paleo constraints on long-term AIS behaviour --- Last Interglacial sea-level highstand implying substantial AIS contribution, Pliocene warm-period constraints on AIS sensitivity to sustained warming --- could provide discriminatory power for A4 scenario weighting even in the absence of a long modern overlap window.  The interface should accept a set of $(t, r_{\text{AIS}}, \sigma)$ triples from any source (modern budget residual, paleo proxy, or hypothetical future observation), so that the consistency likelihood is computed uniformly regardless of data provenance.  This is a Phase~2/3 capability, but the interface must be designed in Phase~1 to avoid retrofit.
...
$\text{Var}_{\boldsymbol{\theta}}[H(t) \mid S, k]$ and $\text{Var}_k[E_{\boldsymbol{\theta}}[H(t) \mid S, k]]$ are approximately independent --- i.e., the posterior parameter uncertainty does not depend strongly on which A4 scenario is assumed.  This holds if the A4 module does not substantially modify the AIS rate during the calibration period, which in turn requires that the A4 scenarios are consistent with the historical budget residual.  The A4/budget consistency diagnostic (\cref{sec:a4-consistency}) tests this condition.  If A4 posterior weights differ substantially from prior weights, Assumption~V1 may be violated.
...
\begin{enumerate}
  ...
  \item For each draw, sample an A4 scenario~$k$ from its posterior-updated categorical distribution (\cref{eq:posterior-weights}).
...
\begin{enumerate}
  ...
  \item \textbf{Scenario weights from process understanding:} The A4 prior mixture weights (Level~3) are informed by process-model ensembles and expert assessment.  The historical calibration updates these weights via the budget consistency diagnostic (\cref{sec:a4-consistency}), but the prior weights are set by process understanding, not by historical fit.  This is appropriate because the deep-uncertainty scenarios represent futures that have not yet been observed.
