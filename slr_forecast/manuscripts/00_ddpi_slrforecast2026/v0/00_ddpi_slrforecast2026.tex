...
For a linear model with Gaussian likelihood and known observation variances, the WLS solution is the Bayesian posterior under flat priors: the posterior mean is the WLS estimate $(X^\top W X)^{-1} X^\top W y$ and the posterior covariance is $(X^\top W X)^{-1}$. Monte Carlo samples are drawn from this Gaussian posterior, with BIC-weighted mixing across candidate delays to propagate delay uncertainty.
...
\begin{enumerate}
	...
	\item[\textbf{S2.}] \textbf{MISI} ($P = 0.80$; 120--1000\,mm; $\alpha = +4$). Marine ice sheet instability is triggered, with the full range of amplifying processes that current models omit. The lower bound (120\,mm) represents late-onset MISI ($\sim$2060) with high residual buttressing, limited primarily to the Thwaites basin. Two independent arguments anchor this value: (i)~rate separation from S1---S1's upper bound (85\,mm) is the maximum without MISI, so the instability feedback must produce rates clearly above the melt-driven linear response, and even late-triggering MISI that doubles the late-century rate gives $\sim\!120$\,mm; (ii)~bed geometry---if MISI drives Thwaites' grounding line through even a portion of its retrograde bed section (bed deepening from $\sim\!700$\,m to $\sim\!2000$\,m; BedMachine, \citealp{Morlighem2020}), partial traverse produces $\sim\!50$--60\,mm\,SLE from Thwaites alone, which combined with melt-driven contributions from other basins exceeds 120\,mm. The upper bound (1000\,mm) matches the Bamber et al.\ structured expert judgment 95th percentile for WAIS under $+5\,^\circ$C \citep[$\sim\!930$\,mm;][]{Bamber2019}. The skewness parameter $\alpha = +4$ is constrained by two requirements: (i)~\citet{Robel2019} showed analytically that MISI produces substantial positive skewness when the grounding-line flux nonlinearity exponent exceeds 3, prescribing $\alpha > 0$; (ii)~the \citet{Bamber2019} conditional-on-MISI median ($\sim\!250$--300\,mm, obtained by removing the $\sim\!15\%$ of outcomes corresponding to stability and $\sim\!5\%$ to MICI extremes from the unconditional distribution) must be reproduced at the $n_{\mathrm{ref}} = 3$ reference rheology. With bounds of 120--1000\,mm, $\alpha = 4$ produces a pre-correction median of $\sim\!263$\,mm, consistent with the Bamber conditional range. After rheology correction ($\times\,1.28$), the median shifts to $\sim\!336$\,mm---higher than Bamber, as expected because the correction captures the $n = 3 \to 4$ bias that postdates the 2018--2019 elicitation \citep{Millstein2022, Martin2026}.
...
S2: MISI         & 0.80 & 120--1000  & $+4$ & LN(1.8, 0.3) & Right-skewed & MISI; accelerating \\
