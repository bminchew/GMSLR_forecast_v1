...
The forecasts presented here use the IPCC AR6 warming trajectories, though they can be applied to any GMST trajectory. We focus on the policy-relevant emission scenarios, which are labeled by their approximate median end-of-century global mean surface temperature rounded to the nearest integer: 2$\,^\circ$C (SSP1-2.6), 3$\,^\circ$C (SSP2-4.5), and 4$\,^\circ$C (SSP3-7.0). We initialize the GMSL forecast using the GMSL level, rate, and acceleration observed in the satellite-era and slowly blend in the component-model sum in rate space, such that
\begin{equation}
...
\end{equation}
The blending is performed sample-by-sample across the 2{,}000-member Monte Carlo ensemble, preserving correlations between the quadratic-fit parameter uncertainty and the component-model uncertainty. Because the quadratic extrapolation is derived entirely from the satellite altimetry record, it carries no dependence on emissions pathway. The component sum, $\dot{H}_{\mathrm{comp}}$, is SSP-dependent through the temperature-forced component models. Consequently, the blended forecast is SSP-independent in the near term (where $w \approx 1$) and becomes progressively SSP-dependent as the component physics dominate at later times.

Uncertainty in $\dot{H}{\mathrm{comp}}$ is propagated by direct Monte Carlo summation. The forecast sum comprises six components, thermosteric expansion, Greenland SMB and discharge, glaciers, the Antarctic Peninsula, WAIS, and terrestrial water storage, each contributing its own $N=2000$-member ensemble of level-space projections $H_k(t)$, drawn independently from that component's own posterior or, for components using prescribed literature sensitivities rather than data calibration (below), from the corresponding parameter distributions. EAIS is not included in the forecast sum; its raw observational record is retained only for the budget-closure diagnostic (\S\ref{s:budget_close}). WAIS is generated with $N=10{,}000$ samples to adequately resolve the low-probability tail of its scenario mixture (below) and is downsampled without replacement to $N=2{,}000$ so that every component shares the ensemble size used in the sum. The total level is formed sample-by-sample, $H{\mathrm{comp},i}(t)=\sum_k H_{k,i}(t)$ for $i=1,\dots,2000$, so draws from independently calibrated components are summed without pairing individual samples by any shared physical driver, consistent with the independence assumption underlying the budget-closure quadrature. The component rate $\dot{H}_{\mathrm{comp}}$ used in Eq.~\eqref{eq:blend_rate} is obtained by finite-differencing this summed ensemble in time, sample-by-sample, before blending with the observational quadratic rate. Reported credible intervals (66\% and 90\%) are the 17th/83rd and 5th/95th empirical percentiles of the resulting ensemble at each time step rather than $\pm k\sigma$ ranges from combined variances, which preserves non-Gaussian features, including the bimodality introduced by WAIS's discrete scenario mixture, that a variance-based combination would otherwise smooth away.
...
\subsubsection{Validation: Hindcasting level, rate, and transient sensitivity}

Any claim to a model's predictive skill must be supported by validation against data. Here, we validate the model by two hindcasting approaches. Both are generated by forcing the model components with observed GMST back to 1950, the limit of reliable GMSL data. The first comparison is between global mean sea level and rate produced by SPECTRUM with the observations from \citet{Fox-Kemper2021} and \citet{Frederikse2020}. For the second comparison, we calculate the transient sensitivity of the SPECTRUM output and the observations, where the transient sensitivity is defined as $\alpha = d\dot{S}/dT$ with $\dot{S} = dGMSL/dt$ the rate of GMSL. Here, we use the same $t_0$ starting point (2025, the end of the satellite altimetry record) and tapering scheme as described in the forecasting section (\S\ref{s:forecasting_model}) to ensure the hindcast differs only from the forecast in the GMST trend used to drive the model. %While a hindcast analysis should be seen as a necessary condition for presenting a forecast, the sensitivity test is arguably the more important validation because a sea-level projection model is simply a way of representing the sensitivity of GMSL to warming to allow for calculations of GMSL given a GMST forcing. 
