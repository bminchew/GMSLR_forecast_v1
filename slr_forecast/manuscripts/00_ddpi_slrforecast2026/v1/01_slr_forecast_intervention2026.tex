...
\caption{\textbf{Two-layer thermosteric model posterior correlation structure.} The joint posterior for the six parameters of the two-layer energy balance model fitted to NOAA 0--700\,m and 0--2000\,m thermosteric sea level data. Diagonal panels show marginal posterior histograms (red), with dashed vertical lines marking the median and 90\% credible interval. Off-diagonal panels show the joint posterior density, with contours denoting the 1, 2, and 3$\sigma$ credible regions. $b_u$ and $\tau_u$ are correlated in the likelihood---a slower upper-layer response can be partly offset by a larger sensitivity to fit the same trajectory---but this partial degeneracy is broken by $\tau_u$'s informative, physically motivated prior (black curve; \citep{Geoffroy2013}). $a$, $b_u$, and $c$ share moderate collinearity as coefficients on the same temperature regressor. Only $\tau_u$ has an informative prior (black curve) while the other five parameters have flat, uninformative priors (not shown). The deep-ocean relaxation time $\tau_d = 150$\,yr is fixed. The posterior for $b_d$ is wider than the others, reflecting the shorter 0--2000\,m record (21 years vs.\ 71 years for 0--700\,m).}
\label{fig:ocean_priors_posteriors}
\end{figure}



We validate the full-depth model in Eq. \ref{eq:thermo_full} against the independently derived reconstruction of \citet{Frederikse2020}, withheld from calibration (Fig. \ref{fig:ocean_fit_residuals}c,f). \citet{Frederikse2020} construct global-mean steric sea level for 1900–2018 by combining in situ subsurface ocean-temperature products over 1957–2018 with a reconstruction of historical ocean heat storage for the pre-1957 period. Our comparison uses their 1957–2018 portion, showing good agreement with a relative RMS of 2.5 mm (Fig. \ref{fig:ocean_fit_residuals}c,f). For completeness, we also show a pre-1957 comparison, where our model and the reconstruction broadly agree, with an RMS of 3.8 mm.  
...
The EAIS component follows the process as for the Peninsula and is fit to the IMBIE EAIS record (1992--2020, 29 annual points; \citealp{Otosaka2023}). This 29-year record shows large interannual variability, and its contribution to GMSL is small compared to other components. As with WAIS, we do not use this rate--temperature fit for projection; instead, we use the raw IMBIE EAIS observations directly in the budget closure (\S\ref{s:budget_close}), with their reported measurement uncertainty, rather than a model fit. 
...
\end{equation}
where $\Phi(x) =(1 + e^{-x})^{-1}$ is the standard logistic function, $t_r = 2035$, and $\tau_r = 5$ yr. At $t \ll t_r$, $w \approx 1$ and the forecast follows the observed quadratic trajectory. At $t \gg t_r$, $w \approx 0$ and the forecast follows the component-model physics. The transition spans approximately 22 yr, meaning that 10\%--90\% of the weight shift occurs over $\pm 2.2 \tau_r \approx \pm 11$ yr around $t_r$. We chose $t_r = 2035$ because it is more than a quarter and less than a half of the GMSL satellite altimetry record into the future, a period when the extrapolation of observations should have strong forecasting skill. The tapering recognizes the expected reduction in forecasting skill of the simple data extrapolation. Pushing $t_r$ to 5 years later (approximately half of the calibration window and arguably the latest viable choice) produces forecasts that are indistinguishable within uncertainty, making the results insensitive to the specific value of $t_r$ within the range of 10-15 years into the future.  

The GMSL forecast is obtained by integrating Eq.~\eqref{eq:blend_rate} forward from the observed level at the forecast origin ($t_0 = 2025$, the end of the NASA altimetry record), so that 
\begin{equation}
	H_{\mathrm{forecast}}(t) = H_{\mathrm{obs}}(t_0) + \int_{t_0}^{t} \dot{H}_{\mathrm{forecast}}(t')\,dt'.
	\label{eq:blend_level}
\end{equation}
The blending is performed sample-by-sample across the 2000-member Monte Carlo ensemble, preserving correlations between the quadratic-fit parameter uncertainty and the component-model uncertainty. Because the quadratic extrapolation is derived entirely from the satellite altimetry record, it carries no dependence on emissions pathway. The component sum, $\dot{H}_{\mathrm{comp}}$, is SSP-dependent through the temperature-forced component models. Consequently, the blended forecast is SSP-independent in the near term (where $w \approx 1$) and becomes progressively SSP-dependent as the component physics dominate at later times.

Uncertainty in $\dot{H}{\mathrm{comp}}$ is propagated by Monte Carlo summation of the components. Each component has its own $2000$-member ensemble of level-space projections $H_k(t)$, drawn independently from that component's own posterior or, for components using prescribed literature sensitivities rather than data calibration, from the corresponding parameter distributions. The total level is formed sample-by-sample, $H{\mathrm{comp},i}(t)=\sum_k H_{k,i}(t)$ for $i=1,\dots,2000$, so draws from independently calibrated components are summed without pairing individual samples by any shared physical driver. The component rate $\dot{H}_{\mathrm{comp}}$ used in Eq.~\eqref{eq:blend_rate} is obtained by finite-differencing this summed ensemble in time before blending with the observational quadratic rate. Reported credible intervals (66\% and 90\%) are the 17th/83rd and 5th/95th empirical percentiles of the resulting ensemble at each time step, which preserves non-Gaussian features.
...
\begin{equation}
	T_{\mathrm{ocean}}(t) = \alpha T_{\mathrm{surface}}^{\mathrm{Gr}}(t) + \beta + \varepsilon_{go}(t),
	\label{eq:ocean_transfer}
\end{equation}
where $T_{\mathrm{surface}}^{\mathrm{Gr}}$ is the Greenland regional surface temperature anomaly and $\varepsilon_{go}$ is a zero-mean residual. Both series are anomalies relative to the 1995--2005 baseline. An OLS fit over $52$ common annual values gives $\alpha = 0.148 \pm 0.027$, $\beta = -0.051 \pm 0.031\,^\circ$C, and residual standard deviation $\sigma_{\varepsilon_{go}} = 0.20\,^\circ$C. The transfer function is calibrated independently of the discharge model parameters $(\gamma, r_0, \delta)$. The two regressions share no free parameters and are fit on different dependent variables.

For projections, SSP GMST trajectories are converted to Greenland regional surface temperature using an Arctic amplification factor $AA= 3.0 \pm 0.5$, consistent with the observed ratio of Greenland-to-global warming over 1970--2021. Greenland regional surface temperatures are the passed through Eq.~\eqref{eq:ocean_transfer} to obtain projected $T_{\mathrm{ocean}}$, which then enters Eq.~\eqref{eq:greenland_dyn} via the delayed integral. Each of the 2,000 Monte Carlo samples draws $\mathrm{AA} \sim \mathcal{N}(3.0, 0.5^2)$, $\alpha \sim \mathcal{N}(0.148, 0.027^2)$, and $\beta \sim \mathcal{N}(-0.051, 0.031^2)$ jointly with $(\gamma, r_0, \delta)$ from the discharge posterior, so that Arctic amplification, transfer-function, and discharge parameter uncertainties all propagate into the projections. A single draw of $(\alpha, \beta, \gamma, r_0, \delta)$ is applied to all SSP scenarios, so that inter-SSP differences isolate the effect of the emissions pathway from parameter uncertainty.

For SMB in Greenland and EAIS, the forecast uses simple thermodynamic models calibrated from regional climate models. For Greenland SMB, the forecast uses a quadratic temperature scaling $\dot{H}_{\mathrm{smb}} = C_{T^2}\Delta T^2 + C_T \Delta T + \dot{M}_0$ with sensitivities drawn from the spread between regional climate models \citep{Fettweis2020, Noel2021, Glaude2024}. The linear sensitivity $C_T \sim \mathcal{N}(-300, 80^2)$~Gt yr$^{-1}$ $^\circ$C$^{-1}$ of GMST encompasses the MAR, RACMO, and HIRHAM estimates. The quadratic sensitivity, $C_{T^2} \sim \mathcal{N}(-50, 30^2)$~Gt yr$^{-1}$ $^\circ$C$^{-2}$, represents the effects of ablation zone expansion and melt-albedo feedback with warming. $\dot{M}_0 = 380$~Gt yr$^{-1}$ is the baseline SMB. For EAIS SMB, we use a linear model ($C_{T^2}=0$) and the literature-derived accumulation sensitivity $C_T = 60 \pm 20$~Gt yr$^{-1}$ $^\circ$C$^{-1}$ of GMST \citep{Frieler2015, Ligtenberg2013}, reflecting the Clausius--Clapeyron scaling of snowfall with warming ($C_T > 0$ corresponds to mass gain, i.e., negative SLR). Data from IMBIE show EAIS in approximate balance, so we set $\dot{M}_0 =0$. The EAIS contribution is small ($-13$ to $-30$~mm at 2100, depending on scenario) relative to the total, meaning the results are largely insensitive to these parameter choices.
...
Any claim to a model's predictive skill must be supported by validation against data. Here, we validate the model by two hindcasting approaches. Both are generated by forcing the model components with observed GMST back to 1900, with an emphasis on post-1950 the limit of reliable GMSL data. The first comparison is between global mean sea level and rate produced by SPECTRUM with the observations from \citet{Fox-Kemper2021} and \citet{Frederikse2020}. For the second comparison, we calculate the transient sensitivity of the SPECTRUM output and the observations, where the transient sensitivity is defined as $\alpha = d\dot{S}/dT$ with $\dot{S} = dGMSL/dt$ the rate of GMSL. Here, we use the same $t_0$ starting point (2025, the end of the satellite altimetry record) and tapering scheme as described in the forecasting section (\S\ref{s:forecasting_model}) to ensure the hindcast differs only from the forecast in the GMST trend used to drive the model. %While a hindcast analysis should be seen as a necessary condition for presenting a forecast, the sensitivity test is arguably the more important validation because a sea-level projection model is simply a way of representing the sensitivity of GMSL to warming to allow for calculations of GMSL given a GMST forcing. 
...
\textbf{(d)} Global glacier contribution: observations (grey) and scenario forecasts from the temperature-driven glacier model.}
