...
Ordered by complexity they instead form a hierarchy, a practice long established
elsewhere in climate science, where ordering models from idealized to comprehensive
has served understanding better than comprehensive models alone
\citep{Held2005,Jeevanjee2017,Maher2019}. Each level can be calibrated and tested against data before the next is
...
rather than against a single preferred one, and agreement between levels becomes a
diagnostic in its own right. Where the levels agree, a result rests on more than one set
...
constrain and identifies the measurement that would settle it. The simplest levels often
...
\citep{Green2015,Makridakis2020}. A hierarchy built this way complements process-based
...
that arrays these approaches by complexity, testing each level against data
...
or disagreement across levels as a diagnostic in its own right, letting the
...
insights come from the simplest level rather than the most complex. Here,
