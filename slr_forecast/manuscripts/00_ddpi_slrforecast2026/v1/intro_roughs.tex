...
each be calibrated and validated against data. It runs to roughly 2.2 pages and
cites 53 keys, and still introduces no new entries to
\texttt{references.bib}. Every one
of those keys is either already used in the manuscript itself or in Versions
1--6 of this file, or was verified for this revision against a local Zotero PDF
(\texttt{Ranganathan2024}, \texttt{Martin2026}, \texttt{Goldberg2026},
\texttt{Morlighem2024}, and \texttt{Edwards2019}).
...
Rates of global mean sea-level rise (GMSLR) have doubled twice since the middle of the
20th century, first over about 16 years and $0.34\ ^\circ$C of warming and again over
about 21 years and $0.41\ ^\circ$C \citep{Frederikse2020,Hamlington2024,Rohde2020}.
...
done about that depends on how well the rise can be forecast.

Forecasts published over four decades span a wide range of methods and outcomes
\citep{Garner2026}. Coastal planning leans on the Sixth Assessment Report of the
Intergovernmental Panel on Climate Change (AR6), which projects 0.4--0.7~m of rise
this century \citep{Fox-Kemper2021,Hirschfeld2023} and builds its Antarctic contribution
from the ISMIP6 ensemble and its emulator, from LARMIP-2, and from structured expert
judgment \citep{Seroussi2020,Edwards2021,Levermann2020,Bamber2019}. Whatever the method,
a 21st-century projection is out of sample with respect to the modern observational
record. The central difficulty is therefore not building a model but calibrating it,
testing it against data withheld from that calibration, and reporting uncertainties in a
form that separates the contribution of each physical process. Uncertainty that cannot
be traced to a source cannot be reduced by targeted observation. A recent community
synthesis makes the same case, calling for projections narrowed by observations and for
uncertainty characterized by its structure and not only by its magnitude
\citep{Felikson2025}.

Models of sea-level rise span a wide range of complexity and their strengths are
complementary. Process-based ice-sheet models solve the equations of ice flow,
deformation, and fracture, and they are where competing hypotheses about grounding-line
retreat, buttressing, and cliff failure can be posed against one another and tested
\citep{Schoof2007,Gudmundsson2013,Pegler2018,HaseloffSergienko2018,Morlighem2024,
Edwards2019}. They also demand more constraints than the observational record can
supply. In hindcasts they depart from reconstructions of historical GMSLR
\citep{Aschwanden2021}, and most projections initialized in 2016 grow the Antarctic Ice
Sheet through the years that overlap the satellite record \citep{Fricker2025}. Explicit
representations of grounding-zone melt are missing from most ice-sheet models, and
outcomes remain sensitive to where a model starts from \citep{Fricker2025}. The assumed
viscosity of ice is less sensitive to stress than ice-shelf observations and
laboratory-to-glacier scaling indicate
\citep{Millstein2022,GetraerMorlighem2025,Ranganathan2024}, and in marine ice sheet
benchmarks that choice alone changes the modeled 100-year sea-level contribution by tens
of percent \citep{Martin2026}. How a model is calibrated matters as much as what physics
it carries. Transient calibration of two ice-sheet models against Thwaites velocity and
surface-elevation data from 2004 to 2017 gives 2067 loss rates that differ by more than
a factor of two depending on which of those observations enter the calibration
\citep{Goldberg2026}. Forecasting research across many fields finds that models carrying
more free parameters than their calibration data can constrain forecast worse than
simpler ones \citep{Green2015,Makridakis2020}.
...
in the quantity being predicted \citep{Nerem2022,Rahmstorf2007,Vermeer2009,Kopp2016},
and they track the observed record at least as well as contemporaneous process-based
projections \citep{Rahmstorf2012}. They give up any account of which processes are
responsible, and the fitted relationship need not hold as individual contributors
respond on different timescales \citep{Church2013}. Between the two sit component-level
models, which separate GMSLR into its physical contributors and calibrate each one
against observations, model output, or expert assessment
...
No one class is at once the most physically detailed, the most transparent, and the most
tightly constrained by data, so treating any one of them as authoritative discards what
the others offer. Ordered by complexity they instead form a scaffolding, and each rung
can be calibrated and tested against data before the next is trusted. An extrapolation
of the observed record sets a scenario-independent reference that any more elaborate
model should be able to reproduce. A semi-empirical fit adds a dependence on warming and
little else, which isolates that sensitivity and makes it testable on its own. A
...
its own record. A process-based model asks which mechanisms within those contributors
are responsible. Calibration and validation are then carried out within the space of
models rather than against a single preferred one, and agreement between rungs becomes a
diagnostic in its own right. Where the rungs agree, a result rests on more than one set
of assumptions. Where they diverge, the divergence localizes what the data do not yet
constrain. The simplest rungs often carry the most transferable insight, because what
they assume is small enough to state plainly and to test directly. A scaffolding built
this way is only as useful as the data anchoring each rung, and it is a complement to
process-based modeling rather than a substitute for it.

Here we construct a data-driven, component-level forecast of 21st-century sea-level
rise. Where the observational record can constrain a contributor to GMSLR, we give it
its own parsimonious physical model and calibrate that model by Bayesian inference
against an independent record spanning decades, among them NOAA thermosteric estimates
...
close the satellite altimetry budget \citep{Beckley2017,Hamlington2020} and reproduce
the level, the rate, and the transient sensitivity of GMSLR since 1950 across roughly
