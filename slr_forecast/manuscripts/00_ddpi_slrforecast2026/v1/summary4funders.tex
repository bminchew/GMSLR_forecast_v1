...
\begin{figure}[t!]
\centering
\includegraphics[width=\textwidth]{wais_probability2100_simple.png}
\caption{Chance of each level of global sea-level rise in 2100, relative to the year 2000, for warming of about 3 $^\circ$C by 2100. The blue curve shows the forecast if Thwaites Glacier remains stable, and the red curve shows the forecast if Thwaites becomes unstable. Dashed lines mark the typical outcome of each. The right-hand scales convert rise into additional people living below flood level, annual adaptation costs, and annual flood damages without adaptation.}
\label{fig:forecast}
\end{figure}

We drive the SPECTRUM models with three warming trajectories that end near 2, 3, and 4 $^\circ$C above pre-industrial temperatures by 2100. Current policies put us on a path to about 3 $^\circ$C. Figure~\ref{fig:forecast} shows the forecast for 2100 under 3 $^\circ$C for each West Antarctic scenario. If Thwaites Glacier remains stable, the typical outcome is 0.65 m of rise relative to the year 2000, with a 90\% range of [0.53, 0.76] m. If Thwaites becomes unstable, the typical outcome is 0.81 m, but the 90\% range widens to [0.63, 1.77] m and the chance of more than 1 m of rise is 27\%. Blending the two scenarios, the typical outcomes are 0.6, 0.8, and 1 m of rise by 2100 under 2, 3, and 4 $^\circ$C. These values are 38 to 44\% higher than the projections of the Intergovernmental Panel on Climate Change, which are known to be biased low. The chance that sea level rises more than 1 m above its pre-industrial level by 2100 is 25\% under 2 $^\circ$C, 44\% under 3 $^\circ$C, and 90\% under 4 $^\circ$C.
