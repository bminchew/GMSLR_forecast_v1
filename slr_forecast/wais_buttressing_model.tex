...
% ====================================================================
\section{Critique: TODO fix}
\label{sec:critique}
% ====================================================================

The following issues were identified in a critical review of this document (March 2026).  Each is classified as structural (model architecture), physics (physical assumptions), or practical (calibration feasibility), and flagged with a severity level.

\subsection{Structural Issues}

\paragraph{S1. Identifiability of the coupled $(\beta, r)$ system [HIGH].}
The model has two coupled ODEs per basin, with $\sim$9 basin-specific parameters constrained by $\sim$28 years of noisy observations.  The rate equation depends on $d\beta/dt$ --- the time derivative of a poorly observed latent variable --- meaning noise in $\beta$ estimates is amplified by differentiation, making $\alpha$ and $g(\beta)$ difficult to constrain independently.  Only PIG has enough observational leverage to constrain the coupled system; for TWG and ``other ASE,'' the parameter space is likely underdetermined.

\textbf{TODO:} Formal identifiability analysis (Fisher information matrix or profile likelihood) before MCMC.  Synthetic data recovery test: simulate forward with known parameters, recover via MCMC, assess which parameter combinations are recoverable.

\paragraph{S2. The structural failure term $\Phi(\beta)$ lumps distinct processes [MEDIUM].}
The sigmoid $\Phi(\beta) = \beta \cdot \sigma((\beta_{\text{crit}} - \beta)/\Delta\beta)$ captures four physically distinct processes (pinning-point loss, shear-margin rupture, calving-front retreat, hydrofracture) in a single function with two parameters.  Each has different dependences on $\beta$, thickness, temperature, and strain rate.  The unimodal activation profile may not represent the actual sequence of structural events.  For example, pinning-point loss is threshold-like (contact is binary), but shear-margin weakening is progressive and depends on strain history (damage accumulation).

\textbf{TODO:} Consider decomposing $\Phi$ into sub-terms with distinct activation thresholds and rates.  Alternatively, document the conditions under which the single-sigmoid approximation is adequate (e.g., if the timescales of the sub-processes are similar).

\paragraph{S3. Double-counting in the rate equation [HIGH].}
The rate equation has two acceleration terms: buttressing-loss-driven ($\alpha \cdot (-d\beta/dt) \cdot g(\beta)$) and MISI geometric feedback ($\gamma \cdot r$).  In the Schoof/Pegler framework, MISI \emph{is} the buttressing--flux feedback: the rate increase from grounding-line retreat on a retrograde bed is mediated through the thickness-dependent flux condition, which is already captured by $g(\beta)$ when $\beta$ decreases due to retreat.

The intended distinction --- buttressing-loss acceleration (shelf thinning $\to$ less backstress $\to$ more flux) vs.\ geometric acceleration (retreat onto deeper bed $\to$ thicker grounding line $\to$ more flux) --- makes physical sense, but both operate through $h_g^{n+1}$ in the Schoof condition.  If $\beta$ decreases as the grounding line retreats (because the shelf configuration changes), then $g(\beta)$ already includes the MISI feedback implicitly.  An explicit $\gamma \cdot r$ term on top risks double-counting.

\textbf{TODO:} Clarify the separation of concerns.  Either: (a)~define $\beta$ as representing \emph{only} shelf-mediated buttressing (not bed-geometry effects), with the $\gamma$ term capturing bed-geometry MISI separately; or (b)~show that $g(\beta)$ does \emph{not} capture the bed-geometry feedback because $\beta$ is defined relative to a fixed reference geometry, and the bed-geometry effect enters only through $\gamma$.  The current text does not make this separation clear.

\paragraph{S4. $x_{\text{GL}}$ is diagnostic but $\mathcal{R}_i(x_{\text{GL}})$ depends on it [MEDIUM].}
The basal resistance $\mathcal{R}_i$ depends on grounding-line position through $C_i(x_{\text{GL}})$ (BedMachine bed profiles), but $x_{\text{GL}}$ is listed as a ``diagnostic'' variable with no governing equation.  Without dynamical $x_{\text{GL}}$ evolution, the bed-geometry feedback that makes MISI work must be parameterised entirely through $\gamma$, which contributes to the double-counting concern (S3).

\textbf{TODO:} Either promote $x_{\text{GL}}$ to a state variable with a governing equation (e.g., from the Schoof retreat rate), or document how $\mathcal{R}_i(x_{\text{GL}})$ is updated as the simulation progresses.

\subsection{Physics Issues}

\paragraph{P1. The irreversibility constraint is too strong [HIGH].}
The constraint $\beta_i(t) \leq \beta_i(s)$ for all $t > s$ prohibits any recovery of buttressing.  While structural failure (calving, margin rupture) is irreversible on decadal timescales, ocean-driven thinning is not.  If ocean forcing decreases (CDW cooling, natural variability), the shelf can thicken and $\beta$ can partially recover.  The observed 2012--2016 PIG deceleration is cited as evidence of reduced forcing, but the irreversibility constraint would prevent the model from representing the associated partial buttressing recovery.

\textbf{TODO:} Separate $\beta$ into reversible (thickness-driven) and irreversible (structural-damage-driven) components: $\beta = \beta_{\text{thickness}} \cdot \beta_{\text{structural}}$, with the irreversibility constraint applying only to $\beta_{\text{structural}}$.  Alternatively, replace the hard constraint with a soft asymmetry: $d\beta/dt$ can be positive (recovery) but at a rate $\ll$ the erosion rate, reflecting that regrowth is slower than thinning.

\paragraph{P2. The prior on $n$ is too narrow [HIGH].}
The parameter summary gives $n \sim \text{Uniform}[3, 4]$, but Millstein et al.\ (2022) report $n = 4.1 \pm 0.4$.  The Uniform$[3, 4]$ prior excludes $n > 4$, which is within the $1\sigma$ range of the observational estimate and where the flux sensitivity is highest ($s = (n+1)/(m+1)$ increases with $n$).  The rest of the project uses $\mathcal{N}(4.1, 0.4^2)$.

\textbf{TODO:} Update to $n \sim \mathcal{N}(4.1, 0.4^2)$, consistent with the A4 framework and the observational constraint.

\paragraph{P3. The sliding-law exponent $m$ matters more than acknowledged [MEDIUM].}
The Pegler exponent $s = (n+1)/(m+1)$ is equally sensitive to $m$ and $n$.  The prior $m \sim \text{Uniform}[1/4, 1/3]$ spans the Weertman ($m = 1/3$) to Coulomb ($m \to 0$) range, but the document treats $m$ as a minor nuisance parameter.  In reality, $s$ ranges from 3.0 ($n=3, m=1/3$) to 5.0 ($n=4, m=1/4$) --- a 67\% range driven substantially by $m$ uncertainty.  The value of $m$ at the grounding line is among the most uncertain parameters in marine ice-sheet dynamics.

\textbf{TODO:} Elevate $m$ to a first-class uncertainty source.  Consider whether $m$ should be basin-specific (the sliding regime may differ between PIG and TWG).  Propagate $m$ uncertainty through $s$ alongside $n$.

\paragraph{P4. $\theta_{\text{ref}}$ introduces fragility [MEDIUM].}
The sensitivity function $g(\beta)$ depends on $(1 - \theta_{\text{ref}} \beta)^{s-1}$.  Since $s \approx 3$--$4$, a 20\% error in $\theta_{\text{ref}}$ produces a $\sim$60--80\% error in $g$ at moderate $\beta$.  The four methods listed for constraining $\theta_{\text{ref}}$ (thickness ratio, F\"urst backstress, adjoint sensitivity, ABUMIP) give different answers, and the spread is not formally propagated.

\textbf{TODO:} Treat $\theta_{\text{ref},i}$ as a calibrated parameter with a prior informed by the spread across Methods 1--4, rather than a fixed input.  Propagate its uncertainty through the sensitivity function.

\subsection{Practical Issues}

\paragraph{PR1. 31 parameters for 28 years of data [HIGH].}
Three basins $\times$ $\sim$9 parameters + 4 shared $=$ $\sim$31 total.  Even with sub-basin decomposition, velocity fields, and shelf-thickness records as additional constraints, this is a high-dimensional inverse problem with limited data.  The constraint hierarchy (Groups 1--5 in \cref{sec:calibration}) is well-structured but no formal information-content analysis is presented.

\textbf{TODO:} Before implementing MCMC, compute the expected Fisher information from the available observational constraints (IMBIE sub-basin rates, shelf-thinning time series, grounding-line retreat).  Identify which parameter combinations are constrained vs.\ prior-dominated.  Consider a hierarchical shrinkage prior on the basin-specific parameters to reduce the effective dimensionality.

\paragraph{PR2. Stochastic terms are underspecified [MEDIUM].}
Both $\epsilon_\beta$ and $\epsilon_r$ are Ornstein--Uhlenbeck processes with amplitudes described qualitatively (``small relative to the forced term'').  For Bayesian calibration, these terms absorb all model--data misfit not explained by the deterministic dynamics.  If too small, the posterior is overconfident; if too large, the deterministic parameters become unidentifiable.

\textbf{TODO:} Specify amplitudes from the actual residual structure of the PIG calibration.  Run the deterministic model at the posterior mode, compute residuals against IMBIE, and use the residual variance and autocorrelation to set $\sigma_\beta$, $\sigma_r$, $\tau_\beta$, and $\tau_r$.

\subsection{Summary}

The model is a genuine theoretical advance in causal structure, and the flux--buttressing exponent synthesis is its strongest contribution.  The critical issues are: (1)~potential double-counting between the $g(\beta)$ and $\gamma \cdot r$ terms (S3), which requires a clear separation of buttressing-mediated vs.\ bed-geometry-mediated acceleration; (2)~the overparameterisation relative to data (PR1), which requires identifiability analysis before MCMC; and (3)~the too-strong irreversibility constraint (P1), which should be relaxed to allow partial recovery.  The $n$ prior (P2) is a straightforward fix.  The model is appropriate as a Phase~2 target but is not ready for operational use without addressing S3, P1, P2, and PR1.
